...
class login(Component):
    ...
    def start(self):
        ...
        if r.status_code != 200:
            text = "يرجى التحقق من إدخال رقم الهاتف الصحيح"
        ...
        if r.status_code != 200:
            text = "أخفق تسجيل دخولك، يرجى إعادة المحاولة لاحقًا أو الاتصال بنا."
            print r.text
            message = self.create_message(text=text)
            return self.respond(message=message, action="next")
        ...
        if r.status_code == 200:
            ...
            text = "تسجيل الدخول تم بنجاح."
            
        else:
            text = "فشل في تسجيل الدخول."
            text += "\n"
            text += "يرجى التأكد من إدخال رقم هاتف صالح."
